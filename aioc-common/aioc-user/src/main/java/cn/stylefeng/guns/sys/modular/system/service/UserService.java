...
import cn.stylefeng.guns.sys.core.shiro.factory.AuthKit;
...
public class UserService extends ServiceImpl<UserMapper, User> {
     ...
     * 修改密码
     *
     * @param usedAuthUser 是否使用通过cookieName=Authorization 获取user,否则使用gunsToken获取user
     * @author fengshuonan
     * @Date 2018/12/24 22:45
     */
    public void changePwd(String oldPassword, String newPassword, Boolean usedAuthUser) {
        Long userId=null;
//        HttpServletRequest request = HttpContext.getRequest();
//        LoginUser ssoLoginUser= (LoginUser) request.getSession().getAttribute(SsoConstants.LOGIN_USER_SESSION);
//        userId=ssoLoginUser.getId();
        userId=getUserIdFromSession(usedAuthUser);

        log.info("getUserIdFromSession():"+userId);
...
//        String md5Hex = MD5Util.encrypt(password + user.getSalt());
    ...
    public List<Map<String, Object>> getUserMenuNodes(List<Long> roleList) {
        return this.getUserMenuNodes(roleList,LoginContextHolder.getContext().getUser());
    }

    public List<Map<String, Object>> getUserMenuNodes(List<Long> roleList,LoginUser loginUser) {
        if (roleList == null || roleList.size() == 0) {
            ...
            List<Map<String, Object>> systemTypes = loginUser.getSystemTypes();
            systemTypes=(systemTypes==null||systemTypes.size()<=0)?authService.user(loginUser.getAccount()).getSystemTypes():systemTypes;
            loginUser.setSystemTypes(systemTypes);
...
//        result.put("name", user.getName());
        ...
        return this.getUserIndexInfo(user);
    }
     ...
     * @Date 2019/10/17 16:18
     */
    public Map<String, Object> getUserIndexInfo(LoginUser user) {
        ...
        List<Map<String, Object>> menus = this.getUserMenuNodes(roleList,user);
    ...
    public List<UserExpDto> selectList() {
        List<UserExpDto> users = baseMapper.selectExportList();
        for (UserExpDto user : users) {
            Long deptId = user.getDeptId();
            user.setDeptName(ConstantFactory.me().getDeptName(DecimalUtil.getLong(deptId)));
            String sex = user.getSex();
            user.setSex(ConstantFactory.me().getSexName(sex));
            String status = user.getStatus();
            user.setStatus(ConstantFactory.me().getStatusName(status));
            user.setPositionName(ConstantFactory.me().getPositionName(DecimalUtil.getLong(user.getUserId())));
        }
        return users;
    }

    /**
     * 从session中获取userId; session:httpSession/map/redis
     * @param usedAuthUser
     * @return
     */
    public Long getUserIdFromSession(boolean usedAuthUser){
        Long userId=null;
        if (usedAuthUser){
            // map/redis 中获取
            userId= LoginContextHolder.getContext().getUser().getId();
            log.info("get userId by LoginContextHolder");
        }else {
            // httpSession 中获取
            HttpServletRequest request = HttpContext.getRequest();
            LoginUser ssoLoginUser= (LoginUser) request.getSession().getAttribute(SsoConstants.LOGIN_USER_SESSION);
            userId=ssoLoginUser.getId();
            log.info("get userId by HttpSession");
        }
        return userId;
    }
}
